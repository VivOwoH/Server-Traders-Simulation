...
int main(int argc, char ** argv) {
    ...
    for (int i = 0; i < num_traders; i++) {
        ...
        usleep(80000); // give some time for trader to connect and suspend
        connect_pipes(i);
    }
...
int rw_trader(int id, int fd_trader, int fd_exchange) {
    ...
    if (num_bytes == -1) {
        ...
        if (strcmp(cmd, CMD_STRING[BUY]) == 0 &&
            ...
            success_order = (args > 1) ? 0 : success_order;
    ...
    for (int i = 0; i < product_num; i++) {
        ...
        while (cont && lowest_sell != NULL) {
            ...
            if (lowest_sell->price == 0 && lowest_sell->qty == 0) {
                ...
                if (lowest_sell == NULL || lowest_sell->price == 0 || lowest_sell->qty == 0)
