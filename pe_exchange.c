...
int main(int argc, char ** argv) {
    ...
    for (int i = 0; i < num_traders; i++) {
        ...
        if (pids[i] != -1 && kill(pids[i], SIGUSR1) == -1) {
...
int rw_trader(int id, int fd_trader, int fd_exchange) {
    ...
    if (num_bytes == -1) {
        ...
        if (strcmp(cmd, CMD_STRING[BUY]) == 0 &&
                    sscanf(line, CHK_BUY_MSG, &order_id, product, &qty, &price, &dummy) == 4) {
            ...
            if (success_order) {
                ...
                order_time++; // increment counter
                    ...
                    sscanf(line, CHK_SELL_MSG, &order_id, product, &qty, &price, &dummy) == 4) {
            ...
            if (success_order) {
                ...
                order_time++; // increment counter
                    ...
                    sscanf(line, CHK_AMD_MSG, &order_id, &qty, &price, &dummy) == 3) {
            ...
            if (success_order) {
                ...
                order_time++; // increment counter
                    ...
                    sscanf(line, CHK_CANCL_MSG, &order_id, &dummy) == 1) {
...
void match_order_report(orderbook_node book, order_node highest_buy, order_node lowest_sell) {
    ...
    if (pids[highest_buy->trader_id] != -1 && kill(highest_buy->pid, SIGUSR1)==-1) {
    ...
    if (pids[lowest_sell->trader_id] != -1 && kill(lowest_sell->pid, SIGUSR1)==-1) {
